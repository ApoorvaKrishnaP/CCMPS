...
const ZONE_META = [
  ...
  { id: 'Z05', name: 'South Wing' }, { id: 'Z06', name: 'Exit 1' },
  { id: 'Z07', name: 'Exit 2' }, { id: 'Z08', name: 'Concourse' }
];
// POST upload route proxying down to FastAPI network socket port
app.post('/api/upload', upload.single('video'), async (req, res) => {
  ...
  // Task 21: process.spawn is gone! We pass path parameters to FastAPI
  res.json({
    message: `Pipeline configuration transferred down to stream engine.`,
    zone,
...
app.post('/api/pipeline-stop', (req, res) => {
  activeZoneName = null;
  pipelineStatus = 'idle';
  res.json({ message: 'Pipeline tracking terminated.' });
});

// SPA fallback
app.get('/{*path}', (req, res) => res.sendFile(path.join(__dirname, 'public', 'index.html')));
app.listen(PORT, () => console.log(`Gateway proxy operating natively on port ${PORT}`));

// /** HTTP GET helper to reach pipeline_api.py using Axios */
// async function proxyGet(urlPath) {
//   try {
//     const response = await axios.get(`${PIPELINE_API}${urlPath}`, { timeout: 2000 });
